				...
				function() {
					...
					$(".previous-month").click(
							function() {
								...
								newYear = getNextYear(currentMonth,
										currentYear, "back");
					...
					$(".category-li")
							.click(
									function() {
										...
										var contentDivX = $(".tab-content-in")
												.position().top;
										var categoryDivX = $(
												".category-filter-div")
												.position().top;
					...
					//populateTransactionNameSelectJson();
			...
			.post(
					...
					function(response) {
						...
						if (message.indexOf("successfully") > -1) {
							...
							var contentDivX = $(".tab-content-in").position().top;
							var categoryDivX = $(".category-filter-div")
									.position().top;

							// alert("content x :" + contentDivX + " " +
							// categoryDivX);
...
function updateBudget(elem, previousValue) {
	...
	if (budget_id.includes("transaction_")) {
		...
		queryString = 'src/AppBundle/Controller/controller_budget.php?createBudget='
				+ budget_id
				+ "&newAmount="
				+ elem.value
				+ "&userId="
		...
		queryString = 'src/AppBundle/Controller/controller_budget.php?updateBudget='
				+ budget_id + "&newAmount=" + elem.value;
	}
	...
	$.get(queryString, function(response) {
		...
		if (message.indexOf("successfully") > -1) {
			...
			// get spent amount
			var totalspentElement = document.getElementById("totalspent_"
					+ budget_id);
			...
			progressBarHtml = '<li  class="' + progressBarClass
					+ '" data-width="' + parseInt(progressBarValue)
					+ '" data-target="100"></li>';
			...
			$('#lbl_message').text("");
			$('#lbl_message').removeClass("alert-success").addClass(
					"display-none");
		} else {
...
function populateTransactionNameSelectJson(category) {
	...
	queryString = "src/AppBundle/Controller/controller_budget.php?getTransactionNames="
			+ encodeURI(category)
			+ "&UserId="
			+ sessionStorage.whyage65_user_id;
	$.get(queryString, function(response) {
		checkSessionStillValid(response);
		element = document.getElementById("select_transactions");
		populateDropdown(element, response, "config.html", addEvent);
...
function populateCategories() {
	...
	queryString = "src/AppBundle/Controller/controller_transaction.php?getCategories=true";
	$.get(queryString, function(response) {
		checkSessionStillValid(response);
		element = document.getElementById("select_categories");
		populateDropdown(element, response, "config.html", addEvent);
